...
Same strategy as bot.py, with cloud-hosting hardening:
  - 8s startup delay so Railway's network stack settles before first Binance call
  - Retry logic (3 attempts, 1s backoff) around every Binance call
  - Startup message always shows the levels section; falls back to a warning
    if all retries fail at boot (levels will populate on next HTF close)
...
def main():
    ...
    print(f"[{ist_str(now_utc())}] Sleeping {STARTUP_DELAY_SECONDS}s for network to settle before first Binance call...")
    ...
    for symbol in cfg["symbols"]:
        ...
        if d:
            ...
            parts.append(f"*{symbol}* levels")
            parts.append("⚠️ unavailable at boot — will populate on next HTF close")
