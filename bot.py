#!/usr/bin/env python3
"""
Sweep + MSB Bot — long AND short setups, full lifecycle.

HTF closes → auto-derive previous session levels from Binance Futures:
  Lows  → PDL / PWL / PML
  Highs → PDH / PWH / PMH

If the closed HTF candle reacts to a level, arm a setup on the paired LTF.

  Long side (support):
    reaction: candle.low <= support
      close > support → RECLAIM  |  close <= support → BREAKDOWN
    LTF sweep: candle.low < swing_low
    MSB: HIGH of last UP-close candle strictly before sweep
    Entry: LTF close > MSB → 🟢 LONG

  Short side (resistance):
    reaction: candle.high >= resistance
      close < resistance → REJECTION  |  close >= resistance → BREAKOUT
    LTF sweep: candle.high > swing_high
    MSB: LOW of last DOWN-close candle strictly before sweep
    Entry: LTF close < MSB → 🔴 SHORT

Timeframe pairs: 1d↔30m | 4h↔15m | 1h↔5m
Level map: 1d→PWL,PML,PWH,PMH | 4h→PDL,PWL,PDH,PWH | 1h→PDL,PDH
TTLs: 1d=7d | 4h=48h | 1h=8h
"""
...
def fetch_last_closed(symbol: str, interval: str) -> dict | None:
    try:
        ...
        print(f"[{ist_str(now_utc())}] fetch failed {symbol} {interval}: {e}")
        return None
...
def fetch_recent_closed(symbol: str, interval: str, limit: int) -> list[dict]:
    try:
        ...
        print(f"[{ist_str(now_utc())}] fetch recent failed {symbol} {interval}: {e}")
        return []
...
def main():
    ...
    for symbol in cfg["symbols"]:
        d = derive_levels(symbol)
        if d:
            sorted_lvls = sorted(d.items(), key=lambda x: -x[1])
            parts.append(f"*{symbol}* levels (high → low)")
            parts.append(
                "```\n"
                + "\n".join(f"{name:<4}  {val:>10}" for name, val in sorted_lvls)
                + "\n```"
            )
